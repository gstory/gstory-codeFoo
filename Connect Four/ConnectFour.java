	...
	class CFAI{
		...
		//THREE
			...
			for( Coord[] redPair : redTwoIAR ){
				if( heights[redPair[0].get_col()] == redPair[0].get_row() ){
					if( is_gap( redPair[0], redPair[1] ) ){

						priorities.add(0, redPair[0].get_col());
					}
				}
			}
			...
			if( blackTwoIAR.size() != 0 ){
				for( Coord[] pairs : blackTwoIAR ){
					...
					if( heights[pairs[0].get_col()] == pairs[0].get_row() ){
						...
						if( is_gap( pairs[0], pairs[1] ) ){
							priorities.add(0, pairs[0].get_col());
						}
						skip = false;
						...
						for( Coord[] redPair : redTwoIAR ){
							...
							if( heights[pairs[0].get_col()] + 1 == redPair[0].get_row() && is_gap( redPair[0], redPair[1] ) ){
				...
				for( Coord[] redPairs : redTwoIAR ){
					...
					if( heights[redPairs[0].get_col()] == redPairs[0].get_row() ){
						...
						if( is_gap( redPairs[0], redPairs[1] ) ){
							...
							priorities.add(0, redPairs[0].get_col());
